...
def get_incident(incident_id: str, db: Session = Depends(get_db)):
    inc = db.query(RoadsideIncident).filter(
        (RoadsideIncident.id == incident_id) | (RoadsideIncident.public_incident_id == incident_id)
    ).first()
    if not inc:
        raise HTTPException(status_code=404, detail="Incident not found")
    return {
...
def list_incidents(
    ...
    q = db.query(RoadsideIncident)
    if organization_id:
        q = q.filter(RoadsideIncident.organization_id == uuid.UUID(organization_id))
    if incident_status:
        q = q.filter(RoadsideIncident.status == IncidentStatus(incident_status))
    incidents = q.order_by(RoadsideIncident.created_at.desc()).limit(limit).all()
    return [
        {
            "id": str(i.id),
            "public_incident_id": i.public_incident_id,
...
def request_location(
    ...
    token = secrets.token_urlsafe(32)
    session = LocationCaptureSession(
        incident_id=inc.id,
        token=token,
        expires_at=datetime.now(timezone.utc) + timedelta(hours=2),
        status=LocationSessionStatus.pending,
    ...
    sms = get_sms_provider("fleet")
    result = sms.send(to=phone, body=f"Roadcall: Share your location so we can send help → {link}")

    if result.success:
        session.status = LocationSessionStatus.link_sent
...
@router.post("/location/submit")
def submit_location(payload: LocationSubmit, db: Session = Depends(get_db)):
    """Called by the browser location page when the driver shares coordinates."""
    session = db.query(LocationCaptureSession).filter(
        LocationCaptureSession.token == payload.token
    ).first()
    if not session:
        raise HTTPException(status_code=404, detail="Token not found")
    ...
    session.accuracy_meters = payload.accuracy_meters
    session.captured_at = datetime.now(timezone.utc)
    session.status = LocationSessionStatus.captured

    # Update linked incident
    if session.incident_id:
        inc = db.query(RoadsideIncident).filter(RoadsideIncident.id == session.incident_id).first()
        if inc:
            inc.breakdown_lat = payload.lat
            inc.breakdown_lng = payload.lng
            inc.location_captured_at = session.captured_at
...
def match_vendors(
    ...
    """Return active vendors near the given coordinates."""
    import math

    vendors = db.query(Vendor).filter(Vendor.is_active == True)
    if state:
        vendors = vendors.filter(Vendor.state == state.upper())
...
def create_vehicle(payload: VehicleCreate, db: Session = Depends(get_db)):
    ...
    v.organization_id = uuid.UUID(payload.organization_id)
    db.add(v)
    db.commit()
    db.refresh(v)
    return {"vehicle_id": str(v.id)}
...
@router.get("/vehicles")
def list_vehicles(organization_id: str, db: Session = Depends(get_db)):
...
@router.get("/drivers")
def list_drivers(organization_id: str, db: Session = Depends(get_db)):
